...
| 🐱 像素桌宠动画 | 内置猫 / 狐狸 / 兔子透明像素立绘与机器人精灵表，均支持四态程序动画 |
...
| 👀 眼睛跟随 | 机器人皮肤的眼球随鼠标转动，约 2~3.8s 随机眨眼 |
...
| ☀️🌙 主题切换 | 浅色（橙白渐变） / 深色（原紫调）一键切换，宠物气泡 / 聊天框 / 设置面板同步换肤 |
| 🎩 装扮系统 | 机器人皮肤支持程序化像素配饰：帽子 / 围巾 / 眼镜 |
...
| 双击 | 打开 AI 对话输入框（未启用 AI 时打开设置；每次聊天回复好感度 +2） |
...
| 主题 | 浅色（橙白渐变）/ 深色（紫调）一键切换，宠物窗口与设置面板同步生效 |
| 宠物类型 | 猫 🐱 / 狐狸 🦊 / 兔子 🐰 / 机器人 🤖，实时切换 |
| 配饰 | 无 / 帽子 🎩 / 围巾 🧣 / 眼镜 👓，程序化像素绘制，仅机器人皮肤显示 |
...
> ⚠️ 注意：自定义外观时「眼睛跟随鼠标」自动关闭（机器人精灵的眼睛是程序叠加绘制的，
> 无法在任意图片/模型上定位面部）；其余交互全部保留。透明背景的图片效果最佳。
> 3D 模式需要显卡支持 WebGL（绝大多数电脑都支持）。

### 1. 替换 / 新增精灵图

猫、狐狸和兔子的透明立绘位于 `src/assets/sprite-sources/`，由程序添加呼吸、行走、睡眠和点击动画；机器人精灵表由 `scripts/generate-sprites.mjs` 程序化生成。

**方式 A：换用你自己的精灵表（推荐）**
直接把图片替换为同名文件即可，无需改代码：

```
src/assets/sprite-sources/cat.png     ← 带透明背景的单张角色立绘
```
...
机器人的眼睛、眨眼与 Zzz 由渲染进程叠加绘制；猫、狐狸和兔子的面部已包含在立绘中。
...
│   └── assets/
│       ├── sprite-sources/    # 内置猫 / 狐狸 / 兔子透明立绘
│       ├── sprites/           # 生成的机器人精灵表及兼容资源
...
- **内置外观数量**：猫 / 狐狸 / 兔子 / 机器人，更多皮肤可参考「自定义指南」。
...
- 内置猫 / 狐狸 / 兔子素材随本仓库分发；机器人精灵由本项目程序化生成。
