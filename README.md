...
| 🐱 像素精灵动画 | 内置猫 / 狗 / 团子 / 机器人四套程序化生成的像素精灵表（原创，无版权问题） |
| 🧊 3D 模型皮肤 | 自定义外观支持 GLB 3D 模型（WebGL 渲染 + 射线点击判定 + 程序化动画） |
| ✂️ 自动抠图 | 导入图片时自动移除纯色 / 简单背景，只保留主体（可开关、可调灵敏度） |
...
| 🎩 装扮系统 | 程序化像素配饰：帽子 / 围巾 / 眼镜，设置里实时切换（内置皮肤） |
...
| 宠物类型 | 猫 🐱 / 狗 🐶 / 团子 👻 / 机器人 🤖，实时切换 |
| 配饰 | 无 / 帽子 🎩 / 围巾 🧣 / 眼镜 👓，程序化像素绘制，内置皮肤实时生效 |
| 自动抠图 | 开关 + 灵敏度滑杆（8~60），仅「单张图片」和「2.5D 立牌」模式生效 |
