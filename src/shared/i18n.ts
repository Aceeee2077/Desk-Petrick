...
export const zhDict: Record<string, I18nValue> = {
  ...
  'settings.aiHint':
    '支持任何 OpenAI 兼容接口：OpenAI、DeepSeek、Moonshot、Ollama 等。<br/>API Key 会使用系统安全存储加密后保存在本机（userData/config.json），不会上传到任何地方。',
...
export const enDict: Record<string, I18nValue> = {
  ...
  'settings.aiHint':
    'Works with any OpenAI-compatible API: OpenAI, DeepSeek, Moonshot, Ollama, etc.<br/>Your API Key is encrypted with OS secure storage before it is saved locally (userData/config.json) and is never uploaded anywhere.',
